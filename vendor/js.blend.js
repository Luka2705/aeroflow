    ...
    function worker_code() {
        ...
        MASTER_SDNA_SCHEMA.prototype = {
            getSDNAStructureConstructor: function(name, struct) {
                if (struct) {
                    ...
                    var offset = 0;
                    // Auch für feldlose Structs registrieren (z.B. "raw_data" im neuen Format)
                    this.SDNA[name] = { constructor: blen_struct };
        ...
        */
        ...
        function readFile() {
            ...
            offset += 7;
            // Neues Format (Blender 4.5+): "BLENDER17-01v0500" — 17-Byte-Header,
            // 32-Byte-Blockheader (LargeBHead8), 4-stellige Version
            var large_bhead = false;
            var version;
            if (toString(_data, offset, offset + 2) === "17") {
                large_bhead = true;
                pointer_size = 8;
                BIG_ENDIAN = true; // Flag wird in js.blend als littleEndian-Parameter genutzt
                version = toString(_data, offset + 6, offset + 10); // nach "17-01v"
                offset += 10;
            } else {
            ...
            // Blockheader-Felder (legacy: code,len32,old,sdna32,nr32 | large: code,sdna32,old64,len64,nr64)
            var BHEAD_SIZE = large_bhead ? 32 : 16 + pointer_size;
            var bheadSDNA = (o) => large_bhead ? data.getInt32(o + 4, true) : data.getInt32(o + pointer_size + 8, BIG_ENDIAN);
            var bheadLen = (o) => large_bhead ? Number(data.getBigInt64(o + 16, true)) : data.getInt32(o + 4, true);
            var bheadCount = (o) => large_bhead ? Number(data.getBigInt64(o + 24, true)) : data.getInt32(o + 12 + pointer_size, BIG_ENDIAN);
            var bheadOldOffset = 8; // 'old'-Pointer liegt in beiden Layouts bei +8 (bei BHead4: ebenfalls +8)
            ...
            if (!current_SDNA_template.SDNA_SET) {
                ...
                while (true) {
                    sdna_index = bheadSDNA(offset2);
                    code = toString(_data, offset2, offset2 + 4).replace(/\u0000/g, "");
                    block_length = bheadLen(offset2);
                    offset2 += BHEAD_SIZE;
                    if (code === "DNA1") {
                        ...
                        // Ausrichtung im DNA-Block ist relativ zum Blockanfang (wichtig fürs neue Format)
                        var dna_base = offset2;
                        var alignDNA = () => { var r = (offset2 - dna_base) % 4; if (r > 0) offset2 += 4 - r; };
                        count = 0;
                        var types = [],
                            fields = [],
                            names = [],
            ...
            while (true) {
                // Neues Format: Blöcke liegen ohne Padding hintereinander
                if (!large_bhead && (offset % 4) > 0) {
                    offset = (4 - (offset % 4)) + offset;
                }

                data_offset = offset;
                sdna_index = bheadSDNA(offset);
                let code_uint = data.getUint32(offset, BIG_ENDIAN);
                offset2 = offset + BHEAD_SIZE;
                offset += bheadLen(offset) + BHEAD_SIZE;
                ...
                if (code_uint === DNA1); //skip - already processed at this point    
                    ...
                    var data_start = data_offset + BHEAD_SIZE;
                    ...
                    if (typeof constructor !== "function") {
                        throw new Error("Kein SDNA-Konstruktor: index=" + sdna_index + " name=" + current_SDNA_template.SDNA_NAMES[sdna_index] + " code=" + toString(_data, data_offset, data_offset + 4) + " namesLen=" + current_SDNA_template.SDNA_NAMES.length);
                    }

                    var size = bheadLen(data_offset);

                    count = bheadCount(data_offset);
